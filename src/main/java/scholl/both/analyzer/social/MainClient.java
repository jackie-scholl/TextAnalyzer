 ...
 * 
 * 
 * if (args.length > 1 && (args[0] == "-f" || args[0] == "--file")) {
 ...
 * //String response =
 * requestURL.replaceAll("callback\\?oauth_token=\\w+&oauth_verifier=(?<verifier>\\w+)",
 * "$g{verifier}");
 * 
 * public static void doNotesStuff(Blog b) { JumblrClient c = b.getClient(); Map<String, Object>
 * options = new
 * HashMap<>(); options.put("reblog_info", "true"); options.put("filter", "text");
 * options.put("notes_info", "true");
 * options.put("id", "49118498864"); options.put("limit", "1");
 * 
 * Post p = c.blogPosts(b.getName(), options).get(0);
 * 
 * List<Note> notes = new ArrayList<Note>(); for (int i = 0; i < p.getNoteCount(); i += 50) {
 * options.put("start", i);
 * options.put("id", p.getId()); Post p2 = b.posts(options).get(0);
 * notes.addAll(Arrays.asList(p2.getNotes())); }
 * 
 * System.out.printf("%s: (%d)%n", p, notes.size()); for (Note n : notes) {
 * System.out.printf("  %s%n", n); }
 * System.out.println();
 * 
 * List<Post> posts = new ArrayList<Post>(); for (Note n : notes) { if (n.getPostId() == null)
 * continue; Map<String,
 * Object> options2 = new HashMap<>(); options2.put("reblog_info", "true"); options2.put("id",
 * n.getPostId()); Post q =
